    ...
    class RepackIosAction < Action
      def self.run(params)
        ...
        if params[:ipa].nil?
          ...
          Gym.config = FastlaneCore::Configuration.create(Gym::Options.available_options, Hash.new)

          output_directory = Gym.config[:output_directory]
          output_name = Gym.config[:output_name]
          project = Gym.config[:project]
        ...
        ipa_original_path = Helper::RepackIosHelper.unpackAndRepack(output_name, ipa_path, params[:contents])

        UI.message("Preparing to sign the new package")
        
        sigh_profile_type = Actions.lane_context[SharedValues::SIGH_PROFILE_TYPE]

        if !sigh_profile_type.nil?
        ...
        if !params[:match_type].nil?
          ...
          provisioning_profile_key = "sigh_%s_%s_profile-path" % [params[:app_identifier], match_type]
        ...
        signing_identity = getSigningIdentityFromProvisionProfile(provisioning_profile)
        ...
        [
         ...
         FastlaneCore::ConfigItem.new(key: :bundle_id,
          ...
          FastlaneCore::ConfigItem.new(key: :contents,
                                       ...
                                       verify_block: proc do |value|
                                          UI.user_error!("No path argument found for the new contents, pass using `contents: 'path-to-new-contents-folder'`") unless (value and not value.empty?)
                                       end),
        ]
      end
